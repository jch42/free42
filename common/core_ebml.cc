...
int sz, j;
    ...
    sz += j;
    if (!fwrite(elBuf, 1, sz, EbmlStateFile)) {                      
        return false;
    }
    if (!fwrite(buf, 1, bufSz, EbmlStateFile)) {                         
        return false;
    }
    return true; 
}
    ...
    if (type == TYPE_REALMATRIX) {
        for (i = 0; i < msz; i++) {
            ...
            if ((sz >= (sizeof(buf) - EbmlPhloatSZ)) || (i == msz - 1)) {
                if (!fwrite(buf, 1, sz, EbmlStateFile)) {
                    return false;
                }
                sz = 0;
            }
...
unsigned char buf[64];
	sz = 0;
    j = ebmlWriteVInt(EBMLFree42ExtensionsVersion, _EBMLFree42ExtensionsVersion, &buf[0]);
    if (j == 0) {
        return false;
    }
    sz += j;
    ...
    sz += j;
    return ebmlFlushSizedEl(EBMLFree42Extensions, -1, sz, buf);
}
...
int j, sz;
    sz = 0;
    j = ebmlWriteVInt(EBMLFree42ExtensionsHpilVersion, _EBMLFree42ExtensionsHpilVersion, &buf[0]);
    if (j == 0) {
        return false;
    }
    sz += j;
    ...
    sz += j;
    return ebmlFlushSizedEl(EBMLFree42ExtensionsHpil, -1, sz, buf);
}
...
unsigned char buf[5];
    nbytes = 0;
    // read first byte
    if (fread(buf, 1, 1, EbmlStateFile) == 1) {
        if (buf[0] & 0x80) {
            if (buf[0] == 0xff) {
        ...
        // read next bytes
        if (nbytes > 1) {
            if (fread(&buf[1], 1, nbytes - 1, EbmlStateFile) != nbytes - 1) {
                nbytes = 0;
            }
        }
        // get result
...
unsigned char buf[8];
    ...
    // read bytes
    if (fread(buf, 1, nbytes, EbmlStateFile) != 0) {
        return -1;
    }
    // get result
    *value = 0;
...
int ebmlGetNext(ebmlElement_Struct * el) {
    elCurrent.elPos = ftell(EbmlStateFile);
    ...
    el->pos = elCurrent.pos = ftell(EbmlStateFile);
    return 0;
}
    ...
    do {
        ...
        if (id == EBMLFree42EOD) {
            if (i == 0) {
                ...
                // seek back to el pos
                fseek(EbmlStateFile, el->pos, SEEK_SET);
                return pos - el->pos;
            }
            else {
                i--;
        ...
        if (len != -1) {
            if ((id & 0x07) != EBMLFree42VIntElement) { 
                // not an VInt !!!
                if (fseek(EbmlStateFile, len, SEEK_CUR) == -1) {
                    return -1;
                }
            }
        }
...
int l;
    ...
    if (el->docId == 0) {
		...
		fseek(EbmlStateFile, 0, SEEK_END);
        elCurrent.docLen = ftell(EbmlStateFile);
		fseek(EbmlStateFile, 0, SEEK_SET);		
    }
    else {
    ...
    if (fseek(EbmlStateFile, el->pos, SEEK_SET) != 0) {
        return -1;
    }
    do {
        elCurrent.elPos = ftell(EbmlStateFile);
        ...
        if ((elCurrent.elId & 0x07) != EBMLFree42VIntElement) { 
            if (fseek(EbmlStateFile, l, SEEK_CUR) != 0) {
                return -1;
            }
        }
...
bool ebmlGetString(ebmlElement_Struct *el, char *value, int len) {
    ...
    if ((len != 0) && (fread(value, 1, len, EbmlStateFile) != len)) {
        return false;
    }
    el->pos = ftell(EbmlStateFile);
    return true;
...
bool ebmlGetPhloat(ebmlElement_Struct *el, phloat *p) {
    ...
    if (fread(b, 1, el->elLen, EbmlStateFile) != el->elLen) {
        return false;
    }
#ifdef BCD_MATH
...
bool ebmlGetProg(ebmlElement_Struct *el, char *value, int *sz) {
    ...
    if (fread(value, 1, *sz, EbmlStateFile) != *sz) {
        return false;
    }
    el->pos = ftell(EbmlStateFile);
    return true;
...
bool ebmlReadElBool(ebmlElement_Struct *el, bool *value) {
    ...
    if (fread(&c, 1, el->elLen, EbmlStateFile) != el->elLen) {
        return false;
    }
    el->pos = ftell(EbmlStateFile);
...
bool ebmlReadElString(ebmlElement_Struct *el, char *value, int *sz) {
    ...
    if ((el->elLen != 0) && (fread(value, 1, el->elLen, EbmlStateFile) != el->elLen)) {
        return false;
    }
    *sz = el->elLen;
    el->pos = ftell(EbmlStateFile);
